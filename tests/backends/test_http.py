...
def test_download_file_from_https(temp_dir, sample_https_url):
    output_path = os.path.join(temp_dir, os.path.basename(sample_https_url))
    HTTPStorage(sample_https_url).download(output_path)
    actual = LocalFileStorage(temp_dir).list(relative=True)
    expected = ['foo.tar.gz', ]
    assert sorted(actual) == sorted(expected)


def test_download_file_from_http(temp_dir, sample_http_url):
    output_path = os.path.join(temp_dir, os.path.basename(sample_http_url))
    HTTPStorage(sample_http_url).download(output_path)
    actual = LocalFileStorage(temp_dir).list(relative=True)
    expected = ['foo.tar.gz', ]
    assert sorted(actual) == sorted(expected)
